...
- A Subquery (or Nested/Inner Query) is a query embedded within another SQL statement. It allows you to use the result of one query as the input for another.
- Syntax Tip: Subqueries are typically enclosed in parentheses () .
- Execution: The inner query executes first and its result is used by the outer (main) query.

## 2. Core Subquery Types

### Single-Row / Scalar Subqueries

- Returns exactly one value (a single column of a single row).
- Example: Finding products with a price higher than the average.
...
- Usage: Often used with comparison operators like =, >, <.

### Multi-Row Subqueries

- Returns a list of values (one column, multiple rows).
- Keywords: IN, NOT IN, ANY, ALL.
...
- A subquery that refers to a column from the outer query. It is executed once for every row processed by the outer query.
- Example: Calculating the total spend for each customer within a SELECT statement.
...
- Why use this instead of JOIN?
    - Simplicity: It allows you to add an aggregate value (like a sum or count) to a list of records without needing a complex GROUP BY clause in the main query.
    - Specific Filtering: It's highly effective when you only need a single piece of summary data from a related table.

## 3. Advanced Filtering: EXISTS and NOT EXISTS

- These operators check for the existence of rows returned by a subquery. They return TRUE or FALSE.
- EXISTS: Used to find records that have a match in another table.
- NOT EXISTS: Used to find "missing" records (e.g., products that have never been ordered).
- Performance Note: EXISTS is often faster than IN on large datasets because it uses short-circuit evaluation - it stops searching as soon as it finds the first match.
- Pro Tip: When using EXISTS, it is standard practice to use SELECT 1 inside the subquery since the actual data returned doesn't matter, only the existence of the row does.

## 4. Subqueries vs. Joins

| Feature | Subqueries | Joins |
|---|---|---|
| Readability | Easier for simple logic (filtering). | Better for combining multiple columns. |
| Data Access | Returns columns only from the outer table. | Can return columns from all joined tables. |
| Performance | Good for existence checks (EXISTS). | Generally more efficient for large-scale data retrieval. |
...
SELECT AVG(total_spent) AS aaverage_customer_spending
...
SELECT AVG(total_spent) AS aaverage_customer_spending
FROM (SELECT customer_id, SUM(total_amount) AS total_spent FROM orders GROUP BY customer_id) AS customer_total;

-- Final query - Find customers who spent more than average
SELECT *, (SELECT SUM(total_amount) FROM orders WHERE customer_id = customers.customer_id) AS total_spent
FROM customers
...
- Unlike regular subqueries which can be executed independently, correlated subqueries are dependent on the outer query and must be re-evaluated for each row processed by the outer query.
